...
def render_model_report_html(
    ...
    *,
    ...
    report_metadata = _markdown_report_metadata(markdown)
    generated_at = _metadata_value(report_metadata, "生成日期", "报告日期") or time.strftime("%Y-%m-%d")
    # Some imported/manual reports contain adjacent Markdown rows on one line
    # (``| ... | | ... |``).  Normalize once here so the HTML renderer never
    # turns several logical records into a single, very wide HTML row.
    ...
    return "\n".join(
        [
            ...
            *[
                ...
                f"<span class=\"hero-meta-key\">{escape(label)}：</span>"
                f"<span class=\"hero-meta-value\">{_inline_markdown_to_html(value)}</span>"
                "</span>"
...
def _markdown_generated_date(markdown: str) -> str | None:
    return _metadata_value(_markdown_report_metadata(markdown), "生成日期", "报告日期")


def _markdown_report_metadata(markdown: str) -> list[tuple[str, str]]:
    """Extract the compact pipe-delimited metadata line below a report title.

    Reports generated by different workflows use slightly different labels, but
    the common form is ``生成日期：… | 版本：… | 模型：…``.  Treating it as
    structured data lets the sidebar and hero show individual, scannable facts
    instead of one long wrapped string.  Table rows are deliberately excluded.
    """
    for line in markdown.splitlines():
        stripped = line.strip()
        if not stripped or stripped.startswith("|"):
            continue
        if "|" not in stripped:
            single = re.fullmatch(r"([^：:]{1,32})[：:]\s*(.+)", stripped)
            if single and single.group(1).strip() in {"生成日期", "报告日期"}:
                return [(single.group(1).strip(), single.group(2).strip())]
            continue
        parts = [part.strip() for part in stripped.split("|") if part.strip()]
        metadata: list[tuple[str, str]] = []
        for part in parts:
            match = re.fullmatch(r"([^：:]{1,32})[：:]\s*(.+)", part)
            if not match:
                metadata = []
                break
        ...
        if metadata and any(label in {"生成日期", "报告日期"} for label, _ in metadata):
            return metadata
    return []


def _metadata_value(metadata: list[tuple[str, str]], *labels: str) -> str | None:
...
def _sidebar_meta(
    ...
    *,
    ...
    items = metadata or [("生成日期", generated_at)]
    if include_gcard_summary:
        compare_label = score_labels.get("gcard_v6", "G卡V6")
        items = [*items, ("对比", f"model_score vs {compare_label}")]
    return "".join(
        '<div class="sidebar-meta-row">'
        f'<span class="sidebar-meta-key">{escape(label)}</span>'
        f'<span class="sidebar-meta-value">{escape(value)}</span>'
        "</div>"
...
def _hero_meta(
    ...
    *,
    ...
    items = metadata or [("生成日期", generated_at)]
    existing_labels = {label for label, _ in items}
    if run_id and not {"Run", "run_id"}.intersection(existing_labels):
        items.append(("Run", run_id))
    label_column = run_config.get("label_column")
    if label_column and "标签" not in existing_labels:
        items.append(("标签", f"`{label_column}`"))
    algorithm = run_config.get("algorithm")
    if algorithm and not {"算法", "模型"}.intersection(existing_labels):
        items.append(("算法", str(algorithm)))
    return items


def _report_nav_items(markdown: str, *, include_gcard_summary: bool) -> list[tuple[str, str]]:
    return [
        (section_id, _nav_label(title))
        for section_id, title in _report_sections(markdown, include_gcard_summary=include_gcard_summary)
    ]


def _report_sections(markdown: str, *, include_gcard_summary: bool) -> list[tuple[str, str]]:
    """Return the ordered, unique section IDs shared by navigation and body.

    The browser wraps each ``h2`` in a section after load.  Keeping the IDs in
    the source document avoids links which point to a different fallback ID
    than the JavaScript wrapper would assign, especially for Chinese headings.
    """
    ...
    for line in markdown.splitlines():
        ...
        if include_gcard_summary and title.startswith("Summary"):
            title = "总结"
        base_id = _section_id_for_title(title, fallback_index=len(sections) + 1)
...
def _section_id_for_title(title: str, *, fallback_index: int) -> str:
    if "Summary" in title or "总结" in title:
        return "summary"
    if "模型描述" in title:
    ...
    return re.sub(r"[^a-z0-9]+", "-", title.lower()).strip("-") or f"section-{fallback_index}"


def normalize_markdown_tables(markdown: str) -> str:
    """Expand accidentally concatenated Markdown table rows.

    A valid Markdown row ends and the next one begins with ``||`` when a
    producer joins lines without a newline.  Splitting that boundary restores
    independent records without changing ordinary multi-line tables or escaped
    literal pipes in cell content.
    """
    ...
    for line in markdown.splitlines():
        ...
        # A legitimate cell may be empty (``| A || B |``).  Treat ``||`` as a
        # row boundary only when the raw row already has more cells than the
        # active header, which is the signature of joined logical records.
        if len(cells) <= header_width:
            normalized_lines.append(line)
            continue

        rows = _split_compacted_table_row(line)
        if len(rows) == 1:
            normalized_lines.append(line)
            continue

        # The joining delimiter can consume a final empty cell (for example
        # ``| metric | || next ...``), so restore that cell from the active
        # table's header width.  Never truncate a row with additional data.
        normalized_lines.extend(_pad_markdown_table_row(row, header_width) for row in rows)
    normalized = "\n".join(normalized_lines)
    return normalized + ("\n" if markdown.endswith("\n") else "")


def _split_compacted_table_row(line: str) -> list[str]:
    stripped = line.strip()
    ...
    for index, part in enumerate(parts):
        ...
        if index > 0 and not candidate.startswith("|"):
            candidate = "| " + candidate
        if index < len(parts) - 1 and not candidate.endswith("|"):
            candidate = candidate.rstrip() + " |"
        if candidate.startswith("|") and candidate.endswith("|"):
            rows.append(candidate)
        else:
            # The line did not have the shape of concatenated table rows; keep
            # it intact rather than guessing at user-authored cell content.
            return [line]
    return rows
...
def _pad_markdown_table_row(row: str, header_width: int) -> str:
    cell_count = len(_markdown_table_cells(row))
    if cell_count >= header_width:
        return row
    return row.rstrip().rstrip("|").rstrip() + " |" * (header_width - cell_count + 1)
...
def _markdown_body_to_report_html(
    ...
    *,
    ...
    section_ids = iter(
        section_id
        for section_id, _ in _report_sections(markdown, include_gcard_summary=include_gcard_summary)
    )

    def next_section_id() -> str:
        try:
            return next(section_ids)
        except StopIteration as exc:  # pragma: no cover - protects future parser changes
            raise ValueError("Markdown heading and report navigation are out of sync") from exc
    ...
    def close_table() -> None:
        nonlocal in_table, table_rows, table_is_summary_cards
        if in_table:
            if table_is_summary_cards:
                html_lines.append(_summary_cards_from_markdown_table(table_rows))
            else:
                ...
                for row_index, cells in enumerate(table_rows):
                    ...
                    html_lines.append(
                        "<tr>"
                        + "".join(f"<{tag}>{_inline_markdown_to_html(cell)}</{tag}>" for cell in cells)
                        + "</tr>"
                    )
                html_lines.append("</table>")
            in_table = False
    ...
    def close_code_block() -> None:
        nonlocal code_fence, code_language, code_lines
        if code_fence is None:
            return
        language_attr = f' class="language-{escape(code_language, quote=True)}"' if code_language else ""
        html_lines.append(f"<pre><code{language_attr}>{escape(chr(10).join(code_lines))}</code></pre>")
        code_fence = None
        code_language = ""
        code_lines = []
    ...
    for line in lines:
        ...
        fence_match = re.fullmatch(r"\s*(`{3,}|~{3,})\s*([^\s]*)\s*", line)
        if fence_match:
            close_ul()
            close_ol()
            close_table()
        ...
        details_match = re.fullmatch(r"\s*:::details(?:\s+(.+?))?\s*", line)
        if details_match:
            close_ul()
            close_ol()
            close_table()
            ...
            nested_details = 0
            for details_line in lines:
                if re.fullmatch(r"\s*:::details(?:\s+.+?)?\s*", details_line):
                    nested_details += 1
                    details_lines.append(details_line)
                    continue
                if re.fullmatch(r"\s*:::\s*", details_line):
                    if nested_details == 0:
                        break
                    nested_details -= 1
                    details_lines.append(details_line)
                    continue
                details_lines.append(details_line)
            label = details_match.group(1) or "展开明细"
            inner_html = _markdown_body_to_report_html(
                "\n".join(details_lines),
                include_gcard_summary=False,
                run_config=run_config,
                eval_dir=eval_dir,
                score_labels=score_labels,
                initial_section_title=current_section_title,
            )
            html_lines.append(
                '<details class="report-details">'
                f"<summary>{_inline_markdown_to_html(label)}</summary>"
                f'<div class="report-details-content">{inner_html}</div>'
                "</details>"
            )
            continue
        ...
        if line.startswith("## "):
            ...
            html_lines.append(
                f'<h2 data-section-id="{escape(next_section_id(), quote=True)}">{_inline_markdown_to_html(title)}</h2>'
            )
            continue
        if line.startswith("### "):
            close_ul()
        ...
        if ordered_match:
            ...
            if not in_ol:
                ...
                html_lines.append(f"<ol{start_attr}>")
                in_ol = True
            html_lines.append(f"<li>{_inline_markdown_to_html(ordered_match.group(2).strip())}</li>")
            continue
        if line.startswith("| ") and line.endswith(" |"):
            ...
            if not in_table:
                in_table = True
                table_is_summary_cards = _is_summary_cards_table(
...
def _is_summary_cards_table(*, current_section_title: str | None, header_cells: list[str]) -> bool:
    """Recognise the compact decision table used at the top of a report.

    The schema is intentionally strict: ordinary three-column evidence tables
    must remain tables, while the canonical Summary table gets a more scannable
    card treatment.
    """
    return (
        current_section_title is not None
        and ("Summary" in current_section_title or "总结" in current_section_title)
        and header_cells == ["维度", "结论", "证据"]
    )


def _summary_cards_from_markdown_table(rows: list[list[str]]) -> str:
    """Render each Summary decision row as a complete, responsive card."""
    cards: list[str] = []
    for row in rows[1:]:
        dimension, conclusion, evidence = (row + ["", "", ""])[:3]
        cards.append(
            '<article class="summary-decision-card">'
            f'<h3 class="summary-decision-title">{_inline_markdown_to_html(dimension)}</h3>'
            '<div class="summary-decision-conclusion">'
            f"{_inline_markdown_to_html(conclusion)}"
            "</div>"
            '<div class="summary-decision-evidence">'
            '<span class="summary-decision-evidence-label">证据</span>'
            f"<p>{_inline_markdown_to_html(evidence)}</p>"
            "</div>"
            "</article>"
        )
    if not cards:
        return '<p class="summary-decision-empty">暂无可展示的总结结论。</p>'
    return '<div class="summary-decision-grid">' + "".join(cards) + "</div>"
...
def _inline_markdown_to_html(text: str) -> str:
    ...
    # Protect inline code before applying emphasis patterns.  This keeps a
    # literal asterisk in ``feature_a*b`` from becoming accidental italic text.
    rendered = re.sub(r"`([^`]+)`", preserve_code, escaped)
    rendered = re.sub(r"\*\*([^*\n]+)\*\*", r"<strong>\1</strong>", rendered)
    rendered = re.sub(r"(?<!\*)\*([^*\n]+)\*(?!\*)", r"<em>\1</em>", rendered)
    rendered = re.sub(r"(?<![\w_])_([^_\n]+)_(?![\w_])", r"<em>\1</em>", rendered)
