---
request_id: 20260709-1710-model-request
title: "复借 G 卡主模型从0重跑"
...
# 合同规范化说明：
# - 原始输入中 data_source_mode 为空；本轮样本为本地 feather，显式声明 local_feather。
# - 原始输入中 sample_location 带 "本地/" 前缀；CLI 需要合法绝对路径，故归一化为 /Users/...。
# - 原始输入中 splits.oot.values 被阶段名污染；按正文修正为 OOT、OOT-OOS。
# - feature_location 不写远端表名，避免误触发真实取数；特征元数据和映射以本轮注册产物为准。
