---
request_id: 20260712-agent-harness-context-pack-remediation
...
reports:
  model_display_name: G卡AgentV2-ContextPack
  score_labels:
    model_score: G卡AgentV2-ContextPack
    gcard_v2: G卡V2
    gcard_v4: G卡V4
    gcard_v5: G卡V5
    gcard_v6: G卡V6
...
# Context Pack 补证验收（v2 专属）

本版本仅作为 Agent Harness Context Pack 证据补证版本，不能修改或覆盖 v1。

- 在首个 `advisor_required` 后，`configs_runtime/train.yaml`、`feature_selection/final_features.txt` 与 `modeling/main_lgbm/tuning_context_round_1.json` 必须在 immutable Context Pack 中标记为 `included`。
- Host-Agent 只能依据 `rmw agent advisor context --json` 返回的 Context Pack 生成调参计划；不得直接读取 version workspace 的 tuning JSON 或其他未打包文件。
- Advisor response、acceptance、consumption receipt 必须绑定同一个 request identity 与 `context_hash`。
- 如 Context Pack 拒绝任何必需调参文件，必须返回 retry/rejected 并等待 replacement request；不得绕过。
